...
def threadCraw(i, countThread):
    ...
    try:
        ...
        saveIMG(pic_name, requests.get(pic_src, headers=header).content)
...
for i in range(init_page,preview_page_cnt):
    #threadCraw(i,countThread)

    #while countThread > TotalThread:
    #    time.sleep(0.5)
    #    print(countThread)
    #_thread.start_new_thread(threadCraw,(i,countThread))
